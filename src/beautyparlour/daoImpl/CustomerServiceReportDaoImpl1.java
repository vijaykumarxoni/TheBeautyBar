...
public class CustomerServiceReportDaoImpl1  implements CustomerServiceReportDao {
    ...
    @Override
     public ArrayList<CustomerServiceReportBean> getCustomerServiceReport(String date) {
        ...
        try {
            String query = "SELECT COUNT(service_id) AS 'services',SUM(price) AS 'price',c.`customer_name`,sd.`sale_date` FROM sale_detail sd INNER JOIN customer c ON c.customer_id=sd.`customer_id` "
                    + "WHERE strftime('%m', sd.`sale_date`) = strftime('%m', '"+date+"') "
                    + "AND strftime('%Y', sd.`sale_date`) = strftime('%Y', '"+date+"' ) GROUP BY sd.`customer_id`";
