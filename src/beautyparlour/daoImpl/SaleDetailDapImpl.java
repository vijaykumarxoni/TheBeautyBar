...
//    public Connection con = SqliteDBCon.LoadDb();
  ...
  @Override
    public ArrayList<SaleDetailBeans> getAllSaleDetailbyCustomerId(int customer_id1,String date ,int saleid ) {
        ...
        try {
            String query = "SELECT  s.`sale_detail_id`,s.`sale_id`,c.`customer_id`,\n" +
"                 ser.`service_id`,\n" +
...
"                    WHERE s.customer_id='"+customer_id1+"' AND s.`sale_date`='"+date+"' AND s.`sale_id`="+saleid+"  ORDER BY s.sale_detail_id DESC";
    ...
    @Override
    public int deleteSaleDetails(int sale_id) {
        int i = 0;
        String query = "delete from sale_detail where sale_id="+sale_id;
        try {
            PreparedStatement ps = con.prepareStatement(query);
    ...
    public int deleteSale(int sale_id) {
        int i = 0;
        String query = "delete from sale where sale_id="+sale_id;
        try {
            PreparedStatement ps = con.prepareStatement(query);
